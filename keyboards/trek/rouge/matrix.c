...
bool matrix_scan_custom(matrix_row_t current_matrix[])
{
  ...
  // JS-16 SW (GP13) → [3,2]
  matrix_row_t sw_bit  = !readPin(GP13) ? (MATRIX_ROW_SHIFTER << 2) : 0;
  matrix_row_t prev_bit = current_matrix[3] & (MATRIX_ROW_SHIFTER << 2);
  if (prev_bit != sw_bit) changed = true;
  current_matrix[3] = (current_matrix[3] & ~(MATRIX_ROW_SHIFTER << 2)) | sw_bit;
